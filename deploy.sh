...
until docker exec miranet-mysql-db mysqladmin ping -uroot -proot --silent &>/dev/null; do
    sleep 2
done
...
docker exec -i miranet-mysql-db mysql -uroot -proot -e "CREATE DATABASE IF NOT EXISTS cacti; CREATE DATABASE IF NOT EXISTS miranet_db;"

# 4. Import Cacti schema dump
if [ -f "cacti_full.sql" ]; then
    echo "📥 Importing Cacti system tables (cacti_full.sql)..."
    docker exec -i miranet-mysql-db mysql -uroot -proot cacti < cacti_full.sql
...
docker exec -i miranet-mysql-db mysql -uroot -proot cacti -e "UPDATE settings SET value = '/var/www/html/cacti/log/cacti.log' WHERE name = 'path_cactilog';"

# 6. Wait for Ollama service to be ready
echo "⏳ Waiting for Ollama AI engine to start..."
until docker exec miranet-ollama curl -s http://localhost:11434/api/tags &>/dev/null; do
    sleep 2
done
